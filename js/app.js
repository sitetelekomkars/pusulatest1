...
async function editSport(title) {
    ...
    const { value: f } = await Swal.fire({
        title: 'Spor Düzenle', html: `<div class="card" style="text-align:left; border-left: 5px solid var(--primary); padding:15px; background:#f8f9fa;">
        <label style="font-weight:bold;">Başlık</label><input id="swal-title" class="swal2-input" value="${s.title}" style="width:100%;">
        <label style="font-weight:bold;">Açıklama</label><textarea id="swal-desc" class="swal2-textarea" style="margin-top:5px;">${s.desc||''}</textarea>
        <label style="font-weight:bold;">İpucu (Tip)</label><input id="swal-tip" class="swal2-input" value="${s.tip||''}" style="width:100%;">
        <label style="font-weight:bold;">Detay (Alt Metin)</label><textarea id="swal-detail" class="swal2-textarea" style="margin-top:5px;">${s.detail||''}</textarea>
        <label style="font-weight:bold;">Okunuşu</label><input id="swal-pron" class="swal2-input" value="${s.pronunciation||''}" style="width:100%;">
        <label style="font-weight:bold;">İkon</label><input id="swal-icon" class="swal2-input" value="${s.icon||''}" style="width:100%;"></div>`,
...
async function editSales(title) {
    ...
    if (!s) return Swal.fire('Hata', 'Bulunamadı.', 'error');
    const { value: f } = await Swal.fire({
        title: 'Satış Düzenle', html: `<div class="card" style="text-align:left; border-left: 5px solid var(--sales); padding:15px; background:#ecfdf5;">
        <label style="font-weight:bold;">Başlık</label><input id="swal-title" class="swal2-input" value="${s.title}" style="width:100%;">
        <label style="font-weight:bold;">Metin</label><textarea id="swal-text" class="swal2-textarea" style="min-height:150px; margin-top:5px;">${s.text||''}</textarea></div>`,
...
async function editNews(index) {
    ...
    const { value: f } = await Swal.fire({
        title: 'Duyuru Düzenle', html: `<div class="card" style="text-align:left; border-left: 5px solid var(--secondary); padding:15px; background:#fff8e1;">
        <label style="font-weight:bold;">Başlık</label><input id="swal-title" class="swal2-input" value="${i.title}" style="width:100%;">
        <div style="display:flex; gap:10px; margin-top:5px;"><div style="flex:1"><label style="font-weight:bold;">Tarih</label><input id="swal-date" class="swal2-input" value="${i.date}" style="width:100%;"></div>
        <div style="flex:1"><label style="font-weight:bold;">Tür</label><select id="swal-type" class="swal2-input" style="width:100%;"><option value="info" ${i.type==='info'?'selected':''}>Bilgi</option><option value="update" ${i.type==='update'?'selected':''}>Update</option><option value="fix" ${i.type==='fix'?'selected':''}>Fix</option></select></div></div>
        <label style="font-weight:bold; margin-top:5px; display:block;">Metin</label><textarea id="swal-desc" class="swal2-textarea" style="margin-top:5px;">${i.desc}</textarea>
        <label style="font-weight:bold; margin-top:5px; display:block;">Durum</label><select id="swal-status" class="swal2-input" style="width:100%;"><option value="Aktif" ${i.status!=='Pasif'?'selected':''}>Aktif</option><option value="Pasif" ${i.status==='Pasif'?'selected':''}>Pasif</option></select></div>`,
